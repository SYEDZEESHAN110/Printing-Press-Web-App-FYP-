...
<div class="form-group" id="payment_reciept_holder" style="display: none;">
    <p class="col-md-12" style="text-align: left;">Upload payment receipt
        <input type="file" name="payment_reciept" id="payment_reciept" required>
    </p>
</div>
...
var bankRecords = <?php echo json_encode($bank_records) ?>;
</script>
...
document.getElementById("shop_select").addEventListener("change", function() {
    ...
    if (shop) {
        ...
        if (bankRecords && typeof bankRecords == "object" && bankRecords.length > 0) {
            let shop_bank_record = bankRecords.find(v => v.shop_id == shop_id);
            if (shop_bank_record) {
                document.getElementById("payment_method").innerHTML += `
                    <option value="bank">Direct Bank Transaction</option>
                `;
            }
        }
    }
...
document.getElementById("payment_method").addEventListener("change", function() {
    ...
    let shop_bank_record = null;
    if (bankRecords && typeof bankRecords == "object" && bankRecords.length > 0) {
        shop_bank_record = bankRecords.find(v => v.shop_id == shop_id);
    }
    if (shop) {
        if (payment_method == "easypaisa") {
            ...
            `;
            document.getElementById("payment_reciept_holder").style.display = "block";
            ...
            if (shop_bank_record) {
                document.getElementById("payment_method_number").innerHTML = `
                Please send directly into our bank account: <b>${shop_bank_record.account_number}</b> <br />
                Account Title: <b>${shop_bank_record.account_title}</b> <br />
                Bank Name: <b>${shop_bank_record.bank_name}</b> <br />
                Amount: ${color_scheme === "RGB" ? RGBCost*quantity : CMYKCost*quantity} <br />
                `;
                document.getElementById("payment_reciept_holder").style.display = "block";
            ...
            `;
            document.getElementById("payment_reciept_holder").style.display = "none";
        }
    } else {
        document.getElementById("payment_method_number").innerHTML = "";
        document.getElementById("payment_reciept_holder").style.display = "none";
