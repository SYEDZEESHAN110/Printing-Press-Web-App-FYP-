...
if (isset($_POST['submit'])) {
    ...
    $payment_receipt = isset($_FILES["payment_reciept"]) ? $_FILES["payment_reciept"] : null;
    ...
    if ($payment_receipt) {
        $payment_receipt_filename = 'upload/' . time() . $payment_receipt['name'];
        move_uploaded_file($payment_receipt['tmp_name'], $payment_receipt_filename);
    }

    $sql = "INSERT INTO print_request(type,paper_quality,size,quantity,color_scheme,phone_number,design,shop_id,payment_method,payment_reciept) VALUES('$services','$paper_quality','$size','$quantity','$color_scheme','$phone_number','$destfile', '$shop_id', '$payment_method', '$payment_receipt_filename')";
